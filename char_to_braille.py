...
def convert_irregular(c):
    ...
    '''
    if is_irregular(c):
        if c in IRREGULAR_CHARS[0]:
            return SPACE
        elif c in IRREGULAR_CHARS[1:3]:
            return HYPHEN
        elif c in IRREGULAR_CHARS[3:6]:
            return APOSTROPHE
        elif c in IRREGULAR_CHARS[6:8]:
            return QUOTES
    else:
        return None
# For corresponding elements in the list IRREGULAR_CHARS,
# looks for similarity with c. If identic, will return the appropriate category 
...
#For any given integer representing decade position,
#its output is its corresponding decade pattern
...
def convert_digit(c):
    ...
    '''
    if is_digit(c):
        return decade_pattern(int(DIGITS[int(c)-2])) + "\n.."
    else:
        return None
# if c is a digit, then it will return the decade pattern corresponding to
# the index c-2 in the DIGITS list and will add a final empty 2-dot line
...
def convert_punctuation(c):
    ...
    if is_punctuation(c):
        ...
        return None
# if c is a punctuation mark, then it will return the 2-dot empty line
# and below the decade pattern corresponding to the index of c in PUNCTUATION
...
def decade_ending(dec_num):
    ...
    if dec_num == 0:
        ...
        return None
# for a given decimal number representing each decade,
#it will return the associated bottow-row
...
def letter_column(c):
    ...
    for decade in LETTERS: 
        if c in decade: 
            return decade.find(c) # will return the decade in which c is
...
def convert_letter(c):
    ...
    if is_letter(c):
        ...
        return None 
# if c is a letter, then it will return the decade pattern for its given column,
# a line break, then the decade ending for its given row.
...
def char_to_braille(c):
    ...
    if is_letter(c):
        ...
        return c
#if c is a letter or an irregular char. or a punctuation mark or a digit, 
# it returns its associated French Braille representation
