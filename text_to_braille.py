...
def is_all_caps(s):
    ...
    if len(s) >= 2:
        for i in range(0, len(s)):
            if (not is_letter(s[i].lower())) or (s != s.upper()):
                return False
        return True
    else:
        return False
    #If a word has a length of 2 or more and if every character is a capital letter
    #then the function returns True and the word is all caps
...
def word_to_braille(word):
    ...
    >>> print_ostring(word_to_braille('"hi"'))
    .. o. .o ..
    oo oo o. oo
    oo .. .. oo
    '''
    ...
    if is_all_caps(word): # if word has all uppercase letters,, add capitalizatiom
        capitalization += CAPITAL + "\n\n" + CAPITAL + "\n\n"
        
        for i in range(0, len(word)):
            translated_word += char_to_braille(word[i]) + "\n\n"
#Returns the given capitalization if word is all caps and the remainder of the translated wor

    else: #If word is not all caps
        for i in range(0, len(word)):

            if is_digit(word[i]): #if a given character in word is a digit
                translated_word += (NUMBER + "\n\n" + char_to_braille(word[i])) + "\n\n"
            
            elif is_letter(word[i]) and word[i] == word[i].upper(): 
                translated_word += CAPITAL + "\n\n" + char_to_braille(word[i]) + "\n\n"
                # given letter is uppercase, not all letters uppercase
            
            elif is_letter(word[i]) or is_irregular(word[i]) or is_punctuation(word[i]): 
                translated_word += char_to_braille(word[i]) + "\n\n"
                # if given character is a lowercase letter, irregular character or punctuation
          
            elif word[i] == '⠰':
                translated_word += '⠰'
                
    return capitalization + translated_word
...
def paragraph_to_unicode(braille_ostring):
    ...
    '⠨⠨⠃⠕⠝⠚⠕⠥⠗ ⠇⠁ ⠍⠕⠝⠙⠑'
    >>> paragraph_to_unicode(paragraph_to_braille('"hi"'))
    '⠶⠓⠊⠶'
    '''
    s = ''
...
def paragraph_to_braille(paragraph):
    ...
    .. .. .. oo .. oo .. .o oo .. .. .. oo o. .o .o
    .o .o .. .. .. o. .. .. o. .. .o .. .. .. .. o.
    >>> print_ostring(paragraph_to_braille('"hi"'))
    .. o. .o ..
    oo oo o. oo
    oo .. .. oo
    '''
...
def new_filename(fname, addition):
    ...
    '''
    return fname[0:len(fname)-4] + "_braille" + fname[len(fname)-4:]
#Assuming that the extension always has three letters, this will insert
# _braille before the dot.extension.
